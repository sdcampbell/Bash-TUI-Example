...
common_commands=(
  ...
  "IAM get users, groups, roles, and policies :: aws iam get-account-authorization-details --profile {PROFILE}"
  "IAM get current user info :: aws iam get-user --profile {PROFILE}"
  "IAM list users :: aws iam list-users"
  "IAM list ssh public keys :: aws iam list-ssh-public-keys --profile {PROFILE}"
  "IAM get ssh public key :: aws iam get-ssh-public-key --user-name {USERNAME} --ssh-public-key-id {PUBLIC_KEY_ID} --encoding SSH"
  "IAM get special permissions of user over specific services :: aws iam list-service-specific-credentials --user-name  {USERNAME} --profile {PROFILE}"
  "IAM get metadata of user, including permissions boundaries :: aws iam get-user --user-name {USERNAME} --profile {PROFILE}"
  "IAM list created access keys :: aws iam list-access-keys --profile {PROFILE}"
  "IAM get inline policies of the user :: aws iam list-user-policies --user-name {USERNAME} --profile {PROFILE}"
  "IAM get inline policy details :: aws iam get-user-policy --user-name {USERNAME} --policy-name {POLICY_NAME} --profile {PROFILE}"
  "IAM get policies of user (it doesn't get inline policies) :: aws iam list-attached-user-policies --user-name {USERNAME} --profile {PROFILE}"
  "IAM get groups :: aws iam list-groups --profile {PROFILE}"
  "IAM get groups of a user :: aws iam list-groups-for-user --user-name {USERNAME} --profile {PROFILE}"
  "IAM get inline policies :: aws iam list-group-policies --group-name {GROUP_NAME} --profile {PROFILE}"
  "IAM get inline policy details :: aws iam get-group-policy --group-name {GROUP_NAME} --policy-name {POLICY_NAME} --profile {PROFILE}"
  "IAM get attached policies :: aws iam list-attached-group-policies --group-name {GROUP_NAME} --profile {PROFILE}"
  "IAM list roles :: aws iam list-roles"
  "IAM get role info :: aws iam get-role --role-name {ROLE_NAME} --profile {PROFILE}"
  "IAM list inline policies :: aws iam list-role-policies --role-name {ROLE_NAME} --profile {PROFILE}"
  "IAM get inline policy details :: aws iam get-role-policy --role-name {ROLE_NAME} --policy-name {POLICY_NAME} --profile {PROFILE}"
  "IAM get attached policies :: aws iam list-attached-role-policies --role-name {ROLE_NAME} --profile {PROFILE}"
  "IAM get role instances :: aws iam list-instance-profiles-for-role --role-name {ROLE_NAME} --profile {PROFILE}"
  "IAM list policies :: aws iam list-policies [--only-attached] [--scope Local] --profile {PROFILE}"
  "IAM get list of policies that give access to the user to the service :: aws iam list-policies-granting-service-access --arn {IDENTITY} --service-namespaces {SERVICE} --profile {PROFILE}"
  "IAM get basic policy info :: aws iam get-policy --policy-arn {POLICY_ARN} --profile {PROFILE}"
  "IAM list policy versions :: aws iam list-policy-versions --policy-arn {ARN} --profile {PROFILE}"
  "IAM get policy version :: aws iam get-policy-version --policy-arn {POLICY_ARN} --version-id {VERSION} --profile {PROFILE}"
  "IAM list SAML providers :: aws iam list-saml-providers --profile {PROFILE}"
  "IAM get SAML provider :: aws iam get-saml-provider --saml-provider-arn {ARN} --profile {PROFILE}"
  "IAM list openid providers :: aws iam list-open-id-connect-providers --profile {PROFILE}"
  "IAM get openid provider :: aws iam get-open-id-connect-provider --open-id-connect-provider-arn {ARN} --profile {PROFILE}"
  "IAM get my identity :: aws sts get-caller-identity --profile {PROFILE}"
  "IAM get user metadata :: aws iam get-user --user-name {USERNAME} --profile {PROFILE}"
  "IAM get info about an access key :: aws iam get-access-key-info --access-key-id {ACCESS_KEY_ID} --profile {PROFILE}"
  "IAM get last time an access key was used :: aws iam get-access-key-last-used --access-key-id {ACCESS_KEY_ID} --profile {PROFILE}"
  "SSO check if IAM Identity Center is used :: aws sso-admin list-instances --profile {PROFILE}"
  "SSO get permissions sets :: aws sso-admin list-permission-sets --instance-arn {INSTANCE_ARN} --profile {PROFILE}"
  "SSO for each permission set, check the permission set info (managed policies + content) :: aws sso-admin describe-permission-set --instance-arn {INSTANCE_ARN} --permission-set-arn {PERMISSION_SET_ARN} --profile {PROFILE}"
  "SSO for each permission set, check the managed policies :: aws sso-admin list-managed-policies-in-permission-set --instance-arn {INSTANCE_ARN} --permission-set-arn {PERMISSION_SET_ARN} --profile {PROFILE}"
  "SSO for each permission set, get the inline policy :: aws sso-admin get-inline-policy-for-permission-set --instance-arn {INSTANCE_ARN} --permission-set-arn {PERMISSION_SET_ARN} --profile {PROFILE}"
  "SSO get accounts where the identities with that permission set are going to have it :: aws sso-admin list-accounts-for-provisioned-permission-set --instance-arn {INSTANCE_ARN} --permission-set-arn {PERMISSION_SET_ARN} --profile {PROFILE}"
  "SSO get users with permission sets :: aws identitystore list-users --identity-store-id {IDENTITY_STORE_ID} --profile {PROFILE}"
  "SSO for each user, get the groups :: aws identitystore list-group-memberships-for-member --identity-store-id {IDENTITY_STORE_ID} --member-id UserId={USER_ID} --profile {PROFILE}"
  "SSO for each user, check if has Permission Sets directly :: aws sso-admin list-permission-sets-provisioned-to-principal --instance-arn {INSTANCE_ARN} --principal-id {USER_ID} --principal-type USER --profile {PROFILE}"
  "SSO for each group, check if has Permission Sets :: aws sso-admin list-permission-sets-provisioned-to-principal --instance-arn {INSTANCE_ARN} --principal-id {GROUP_ID} --principal-type GROUP --profile {PROFILE}"
  "SSO for each user/group with a Permission Set, check in which accounts it'll be used :: aws sso-admin list-account-assignments --instance-arn {INSTANCE_ARN} --account-id {ACCOUNT_ID} --permission-set-arn {PERMISSION_SET_ARN} --profile {PROFILE}"
)
